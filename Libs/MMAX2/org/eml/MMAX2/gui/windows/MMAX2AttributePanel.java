...
public class MMAX2AttributePanel extends JPanel
{
    ...
    public ArrayList<MMAX2Attribute> lastValidAttributes=null;
    
    public ArrayList<String> keepables;
    ...
    public final MMAX2AnnotationScheme getAnnotationScheme() 
    {
    	return this.scheme;
    }
...
//    public final ArrayList<MMAX2Attribute> getAllAttributes()
//    {
...
//    }
    ...
    {    
        ...
        keepables = new ArrayList<String>();        
        ...
        if (MMAX2.getStandardFont() != null) { suppressCheck.setFont(MMAX2.getStandardFont().deriveFont((float)9));}
        ...
        new java.awt.event.MouseAdapter()
            {
                public void mouseEntered(java.awt.event.MouseEvent me) { schemeCopy.annotationHintToFront();}
                public void mouseExited(java.awt.event.MouseEvent me)  { schemeCopy.annotationHintToBack();}
            });        
        
        warnOnExtraAttributes = new JCheckBox("Warn on extra attributes");
        if (MMAX2.getStandardFont() != null){ warnOnExtraAttributes.setFont(MMAX2.getStandardFont().deriveFont((float)9)); }
        ...
        lastValidAttributes = new ArrayList<MMAX2Attribute>();       
        
        modifiablePanel = new JPanel();
        Box tempbox = Box.createHorizontalBox();        
        Box attributeBox = Box.createVerticalBox();
                
        for (int u=0;u<initialAttributes.length;u++) { attributeBox.add(initialAttributes[u]); }
    ...
    /* Receives an attribute name on the current level and returns its normalized name, as defined in the annotation scheme. */ 
    public String normalizeAttributeName(String name)
    {
    	return scheme.normalizeAttributeName(new String(name));
    }
    ...
    {   
    	// This is the Attribute panel level
...
//        for (int e=0;e<currentAttributes.length;e++)
//        {
//        	System.err.println(currentAttributes[e].getDisplayName());
//        	System.err.println(" "+currentAttributes[e].getSelectedValue());
//        }
        ...
        rebuild();
        if (container.isToFront())  { container.toFront(); }
        setApplyEnabled(false);
        setUndoEnabled(false);
    ...
    public final HashMap<String, String> getAttributeValuePairs()
    {
    ...
    public final ArrayList<MMAX2Attribute> getAllCurrentAttributes()
    {
    ...
    {                           
        ...
        HashMap<Object, Object> oldAttributes = new HashMap<Object, Object>(markable.getAttributes());
                
        // Get current state of AttributeWindow, incl. *all* types of attributes
        ArrayList<MMAX2Attribute> currentAttributes = getAllCurrentAttributes();
        MMAX2Attribute currentAttribute = null;
        ...
        for (int z=0;z<currentAttributes.size();z++)
        {
            ...
            {
                System.out.println("Skipping frozen "+currentAttribute.getDisplayName());
                // Store name of frozen attribute so it can be kept 
                frozenNames.add(currentAttribute.getDisplayName());
                continue;
            }
                        
            // At this point, all unfrozen levels are available ??
            // 1.15: Here, we know that currentAttribute is not frozen
            /* Get current attribute name */
            String currentAttributeString = currentAttribute.getDisplayName();
            /* Get value of current attribute */
            // This will return the value that is currently displayed!!
            
            String currentValue = currentAttribute.getSelectedValue();
//            System.err.println(currentAttributeString+" "+currentValue);
            ...
            // For 1.15
            //newAttributes.put(new String(currentAttributeString.toLowerCase()), new String(currentValue));
            newAttributes.put(currentAttributeString, currentValue);
        ...
        Iterator<Object> allAttributeStrings = ((Set<Object>)oldAttributes.keySet()).iterator();
        ...
        while (allAttributeStrings.hasNext())
        {
            ...
            if (scheme.isDefined(currentAttributeString)==false) 
            {
                ...
                //newAttributes.put(new String(currentAttributeString.toLowerCase()), new String(((String) oldAttributes.get(currentAttributeString.toLowerCase()))).toLowerCase());
                // For 1.15
//            	System.err.println(currentAttributeString);
//            	System.err.println(oldAttributes);
            	newAttributes.put(currentAttributeString.toLowerCase(), (String)oldAttributes.get(currentAttributeString));            	
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (int u=0;u<lastValidAttributes.size();u++)
                    {
                        ...
                        MMAX2Attribute currentCurrentAttribute = lastValidAttributes.get(u);                        
                        // Check if its removal has consequences for any existing relations
                        if (currentCurrentAttribute.getDisplayName().equalsIgnoreCase(currentAttributeString))
                        {                            
        ...
        if (scheme.isDebug()) System.err.println("After setMarkableAttributes(): "+markable.getAttributes());        
        ...
        ArrayList<MMAX2Attribute> visibleAttributes = getAllCurrentAttributes();
